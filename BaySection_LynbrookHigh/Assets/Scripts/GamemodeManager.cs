...
public class GamemodeManager : MonoBehaviour {
    ...
    public void ChangeColor(string mode) {
        if (mode == "challenge") {
            ...
            _description.text = "You can play each level starting from level 1 to compete in the global leaderboard. \n \nThis game mode has: \n3 Lives \nMove Count \nGlobal Leaderboard";
        } else if (mode == "relaxed") {
            _challenge.DOColor(Color.white, 0.1f);
            _relaxed.DOColor(_green, 0.1f);
            _title.text = "Casual Play";
            _description.text = "In this gamemode, you can practice each level to compete in the global leaderboard in \"Hardcore mode\". \n \nThis game mode has: \nUnlimited Lives \nLevel Selector";
        }
    }
}
